 ...
 */
template <typename T> struct is_matrix_type_valid : std::false_type {};
...
private:
  ...
  Matrix(size_t rows, size_t cols, const std::string &name = "unnamed")
      : m_numOfRows(rows), m_numOfCols(cols), m_name(name),
        m_lastInstruction("none"), m_data(rows * cols) {}
...
public:
  ...
  static Matrix<T>
  fromList(std::initializer_list<std::initializer_list<T>> rowsList,
           const std::string &name = "unnamed") {
   ...
   */
  static Matrix<T> fromVector(const Vector<T> &vectorToMatrix, size_t rows,
                              size_t cols) {
    if (rows * cols != vectorToMatrix.getSize()) {
      throw std::runtime_error(
          "Dimentions of Matrix don't match with number of elements in vector");
    }
  ...
  static Matrix<T> transpose(const Matrix<T> &matrixToTranspose) {
    Matrix<T> transposedMatrix = zeros(matrixToTranspose.getNumberOfColums(),
                                       matrixToTranspose.getNumberOfRows(),
                                       matrixToTranspose.getName() + "^T");
    transposedMatrix.setInstruction("transpose");
    for (size_t row = 1; row <= matrixToTranspose.getNumberOfRows(); row++) {
      for (size_t column = 1; column <= matrixToTranspose.getNumberOfColums();
           column++) {
  ...
  void transpose() {
    ...
    for (size_t row = 1; row <= m_numOfRows; row++) {
      for (size_t column = 1; column <= m_numOfCols; column++) {
        newData[(column - 1) * m_numOfRows + (row - 1)] =
            m_data[(row - 1) * m_numOfCols + (column - 1)];
      }
    }
  ...
  void addRow(const Vector<T> &row) {
    ...
    if ((row.getSize() != m_numOfCols) && m_numOfCols != 0 &&
        m_numOfRows != 0) {
      throw std::runtime_error("Length of columns dont match up");
    }
    TRACK_INSTRUCTION();
  ...
  void addColumn(const Vector<T> &col) {
    if ((col.getSize() != m_numOfRows) && m_numOfCols != 0 &&
        m_numOfRows != 0) {
      throw std::runtime_error("Length of rows dont match up");
    }
    TRACK_INSTRUCTION();
  ...
  void addColumn(const Matrix<T> &col) {
    ...
    for (size_t row = 0; row < m_numOfRows; row++) {
      size_t insertIndex =
          row * (m_numOfCols + col.getNumberOfColums()) + m_numOfCols;
      m_data.insert(m_data.begin() + insertIndex,
                    col.getData().begin() + row * col.getNumberOfColums(),
                    col.getData().begin() +
                        (row + 1) * col.getNumberOfColums());
    }
  ...
  Matrix<T> minorMatrix(size_t omitRow, size_t omitColumn) const {
    ...
    if (omitRow <= 0 || omitRow > m_numOfRows || omitColumn <= 0 ||
        omitColumn > m_numOfCols) {
      throw std::runtime_error(
          "Index out of bound when calculation matrix minor");
    }
   ...
   */
  size_t getNumberOfElements() const { return m_data.size(); }

  /**
   * @brief Returns the number of columns.
   * @return Number of columns.
   */
  size_t getNumberOfColums() const { return m_numOfCols; }

  /**
   * @brief Returns the number of rows.
   * @return Number of rows.
   */
  size_t getNumberOfRows() const { return m_numOfRows; }
   ...
   */
  bool isSqure() const { return m_numOfCols == m_numOfRows; }
  ...
  void printAll() const {
    std::cout << "-----" << std::endl;
    std::cout << "Matrix name: " << m_name << ", Dimension: " << m_numOfRows
              << "x" << m_numOfCols << std::endl;
    std::cout << "Last operation performed on Matrix: " << m_lastInstruction
              << std::endl;
    std::cout << toString() << std::endl;
    std::cout << "-----" << std::endl;
  }
...
auto elementwise(const Matrix<T> &left, const Matrix<U> &right, Op operation) {
  if ((left.getNumberOfRows() != right.getNumberOfRows()) ||
      (left.getNumberOfColums() != right.getNumberOfColums())) {
    throw std::runtime_error("Numbers of columns or rows don't match up");
  }

  using resultType = decltype(operation(T(), U()));
  Matrix<resultType> resultMatrix = Matrix<resultType>::zeros(
      left.getNumberOfRows(), left.getNumberOfColums());
  for (size_t row = 1; row <= left.getNumberOfRows(); row++) {
    for (size_t column = 1; column <= left.getNumberOfColums(); column++) {
      resultMatrix(row, column) =
          operation(static_cast<resultType>(left(row, column)),
                    static_cast<resultType>(right(row, column)));
    }
  }
  return resultMatrix;
}
...
auto operator*(const Matrix<T> &left, const Matrix<U> &right) {
  ...
  Matrix<resultType> resultMatix = Matrix<resultType>::zeros(
      left.getNumberOfRows(), right.getNumberOfColums(),
...
auto operator*(const Matrix<T> &left, const U &scalar) {
  using resultType = decltype(T() * U());
  Matrix<resultType> resultMatrix = Matrix<resultType>::zeros(
      left.getNumberOfRows(), left.getNumberOfColums(), left.getName());
  for (size_t row = 1; row <= left.getNumberOfRows(); row++) {
    for (size_t column = 1; column <= left.getNumberOfColums(); column++) {
...
bool isMatrixEqual(const Matrix<T> &first, const Matrix<U> &second) {
  if ((first.getNumberOfColums() != second.getNumberOfColums()) ||
...
Matrix<U> castMatrix(const Matrix<T> &matrix) {
  Matrix<U> matrixToReturn =
      Matrix<U>::zeros(matrix.getNumberOfRows(), matrix.getNumberOfColums());
